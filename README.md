...
About CS 340 CRUD_Python_Module and ProjectTwoDashboard
How do you write programs that are maintainable, readable, and adaptable?
I write maintainable programs by separating different responsibilities into their own components and using clear naming, comments, and consistent structure. The CRUD Python module I developed for Project One was particularly useful because it separated the operations from the dashboard code. Instead of placing MongoDB queries throughout the dashboard, the dashboard could interact with the AnimalShelter class and use its CRUD methods. This made the code easier to understand and allowed the same database module to be reused for multiple operations. For example, the module could be used by another app that needed to create, retrieve, update, or delete animal records without having to rewrite the MongoDB connection and database logic. In the future, I could adapt the same approach to other databases or apps by changing the model layer while keeping the rest of the app mostly independent.

How do I approach a problem as a computer scientist?
I approach problems by breaking them into smaller, manageable components and testing each part before combining them. For the project, I developed the database and CRUD functionality first, then built the dashboard around it by adding the data table, filtering options, and charts. This project required more consideration of how multiple components worked together than some of my previous assignments. In future projects, I would continue starting with the client's requirements, developing and testing the underlying data operations, and then building the interface around them.
